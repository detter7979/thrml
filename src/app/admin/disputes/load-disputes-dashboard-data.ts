...
import type { DisputeDecisionRow, TicketWithDecision } from "./disputes-client"
...
export type DisputesDashboardPayload = {
  ticketsWithDecisions: TicketWithDecision[]
  stats: {
    todayOpen: number
    pendingHuman: number
...
export async function loadDisputesDashboardData(admin: AdminClient): Promise<DisputesDashboardPayload> {
  const now = new Date()
