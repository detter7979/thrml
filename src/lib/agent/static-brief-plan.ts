...
export function targetFormatsForBrief(brief: {
  ...
  if (fromSc.length) return Array.from(new Set(fromSc))

  if (typeof brief.format !== "string") return ["1x1"]
...
export function buildOutFormatsForAsset(
  brief: { format?: string | null; success_criteria?: Record<string, unknown> | null },
