...
begin
  if to_regclass('public.profiles') is not null then
    ...
    if has_user_id then
      owner_predicate := '(id = auth.uid() or user_id = auth.uid())';
    else
      owner_predicate := '(id = auth.uid())';
    end if;

    execute 'alter table public.profiles enable row level security';
    execute 'drop policy if exists profiles_self_select on public.profiles';
    execute 'create policy profiles_self_select on public.profiles for select to authenticated using ' || owner_predicate;
    execute 'drop policy if exists profiles_self_insert on public.profiles';
    execute 'create policy profiles_self_insert on public.profiles for insert to authenticated with check ' || owner_predicate;
    execute 'drop policy if exists profiles_self_update on public.profiles';
    execute 'create policy profiles_self_update on public.profiles for update to authenticated using ' || owner_predicate || ' with check ' || owner_predicate;
